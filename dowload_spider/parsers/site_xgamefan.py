...
class BaseParser(ABC):
    ...
    @abstractmethod
    def extract_download_info(self, html: str) -> str:
        """ 提取基本信息 """
        pass
...
class MinixiaoshuoParser(BaseParser):
    def extract_download_info(self, html):
        response = requests.get(html, headers=self.request_headers)
        response.raise_for_status()
        soup = BeautifulSoup(response.text, 'html.parser')
        book_name = soup.select("h1")[0].text.strip()
        return book_name.encode('latin1').decode('utf-8')
    
    def extract_file_url(self, html):
        response = requests.get(html, headers=self.request_headers)
        response.raise_for_status()
        soup = BeautifulSoup(response.text, 'html.parser')
        options = soup.select('div.d-flex.justify-content-center a')
        pages = [opt['href'] for opt in options]
        for path in pages:
            full_url = f"https://www.xgamefan.com{path}"
        
        print(full_url)
        response = requests.get(full_url, headers=self.request_headers)
        response.raise_for_status()
        soup = BeautifulSoup(response.text, 'html.parser')
        options = soup.select('div.content-area div.container a')
        pages = [opt['href'] for opt in options]
        for path in pages:
            index = path[path.find('id='):]
            full_url = f"https://www.xgamefan.com/down/?{index}"
        
        print(full_url)
        response = requests.get(full_url, headers=self.request_headers)
        response.raise_for_status()
        soup = BeautifulSoup(response.text, 'html.parser')
        options = soup.select('div.download_link a')
        pages = [opt['href'] for opt in options]
        print(pages)
...
if __name__ == "__main__":
    ...
    url = f"https://www.xgamefan.com/info/XLOOKGAME007133732.html"
    chapters = parser.extract_file_url(url)
    print(chapters)
